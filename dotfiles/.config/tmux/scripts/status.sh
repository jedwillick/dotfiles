...
win_active=" #I #W#{?#{==:#W,nvim},:#{b:pane_current_path},}${current_flags} "

win_inactive_left="#[fg=${white}]#[bg=${gray}]"
win_inactive_right=""
win_inactive=" #I #W#{?#{==:#W,nvim},:#{b:pane_current_path},}${flags} "
...
tmux set -g status-right "#[fg=${gray},bg=${gray},nobold,nounderscore,noitalics]${right_sep}#[fg=${green},bg=${gray}] #(${TMUX_CONFIG_PATH}/scripts/spotify.sh) "
tmux set -ga status-right "#[fg=${green},bg=${gray},nobold,nounderscore,noitalics]${right_sep}#[fg=${dark_gray},bg=${green}]#(${TMUX_CONFIG_PATH}/scripts/git.sh) "
tmux set -ga status-right "#[fg=${orange},bg=${green}]${right_sep}#[fg=${dark_gray},bg=${orange}] {#(wakatime-cli --today)}"
tmux set -ga status-right "#[push-default]#(${TMUX_PLUGIN_MANAGER_PATH}/tmux-mem-cpu-load/tmux-mem-cpu-load --powerline-right --interval ${interval} --graph-lines 4) "
